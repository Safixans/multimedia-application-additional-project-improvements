...
class PatchSampler:
    ...
    def sample_nearest_patch(self, coords, num_samples, tree=None):
    ...
    def __call__(self, coords, tree=None):
        total_samples = max(self.min_samples, int(len(coords) * self.distribution_func()))
        return self.sample_nearest_patch(coords, total_samples, tree=tree)
